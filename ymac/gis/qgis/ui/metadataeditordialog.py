...
class MetadataEditorDialog(QDialog):
    ...
    def __init__(self, mode, layer):
        ...
        if mode == MetadataEditorDialog.CREATE:
            ...
            try:
                xmlDefaultLocation = Tools.getPluginPath() + "resources\\metadata\default.xml"
...
# TODO "T:\757-Information Management Branch\GIS Group\METADATA\ArcGIS_Metadata\Metadata_Contacts.txt"
        ...
        contactFormWidget.layout().addRow("Contact Organisation",
                                          self.contactOrganisationLineEdit)

        self.contactPositionLineEdit = QLineEdit()
        contactFormWidget.layout().addRow("Contact Position",
                                          self.contactPositionLineEdit)

        self.contactMailAddressLineEdit = QLineEdit()
        contactFormWidget.layout().addRow("Mail Address",
                                          self.contactMailAddressLineEdit)

        self.contactSuburbLineEdit = QLineEdit()
        contactFormWidget.layout().addRow("Suburb/Locality",
                                          self.contactSuburbLineEdit)

        self.contactCountryStateLineEdit = QLineEdit()
        contactFormWidget.layout().addRow("Country/State",
                                          self.contactCountryStateLineEdit)

        self.contactPostcodeLineEdit = QLineEdit()
        contactFormWidget.layout().addRow("Postcode",
                                          self.contactPostcodeLineEdit)

        self.contactTelephoneLineEdit = QLineEdit()
        contactFormWidget.layout().addRow("Telephone",
                                          self.contactTelephoneLineEdit)

        self.contactFaxLineEdit = QLineEdit()
        contactFormWidget.layout().addRow("Fax",
                                          self.contactFaxLineEdit)

        self.contactEmailLineEdit = QLineEdit()
        contactFormWidget.layout().addRow("Email",
                                          self.contactEmailLineEdit)
...
# access tab
        ...
        accessFormWidget.layout().addRow("Stored Data Format",
                                         self.accessStoredDataFormatLineEdit)

        self.accessAccessConstraintsTextEdit = QTextEdit()
        accessFormWidget.layout().addRow("Access Constraints",
                                         self.accessAccessConstraintsTextEdit)
...
# quality tab
        ...
        qualityFormWidget.layout().addRow("Lineage",
                                          self.qualityLineageTextEdit)

        self.qualityPositionalAccuracyTextEdit = QTextEdit()
        qualityFormWidget.layout().addRow("Positional Accuracy",
                                          self.qualityPositionalAccuracyTextEdit)

        self.qualityAttributeAccuracyTextEdit = QTextEdit()
        qualityFormWidget.layout().addRow("Attribute Accuracy",
                                          self.qualityAttributeAccuracyTextEdit)

        self.qualityLogicalConsistancyTextEdit = QTextEdit()
        qualityFormWidget.layout().addRow("Logical Consistancy",
                                          self.qualityLogicalConsistancyTextEdit)

        self.qualityCompletenessTextEdit = QTextEdit()
        qualityFormWidget.layout().addRow("Completeness",
                                          self.qualityCompletenessTextEdit)
...
# status tab
        ...
        statusFormWidget.layout().addRow("Progress",
                                         self.statusProgressTextEdit)

        self.statusMaintenanceUpdateTextEdit = QTextEdit()
        statusFormWidget.layout().addRow("Maintenance/Update",
                                         self.statusMaintenanceUpdateTextEdit)
...
# additional tab
        ...
        geographicExtentGroupBox.layout().addRow("Left",
                                                 self.geographicExtentLeftLineEdit)
        self.geographicExtentLeftLineEdit.setReadOnly(True)

        self.geographicExtentRightLineEdit = QLineEdit()
        geographicExtentGroupBox.layout().addRow("Right",
                                                 self.geographicExtentRightLineEdit)
        self.geographicExtentRightLineEdit.setReadOnly(True)

        self.geographicExtentTopLineEdit = QLineEdit()
        geographicExtentGroupBox.layout().addRow("Top",
                                                 self.geographicExtentTopLineEdit)
        self.geographicExtentTopLineEdit.setReadOnly(True)

        self.geographicExtentBottomLineEdit = QLineEdit()
        geographicExtentGroupBox.layout().addRow("Bottom",
                                                 self.geographicExtentBottomLineEdit)
        ...
        additionalMetadataForm.addRow("Metadata Date",
                                      self.additionalMetadataDateLineEdit)
        self.additionalMetadataDateLineEdit.setEnabled(False)

        self.additionalAdditionalMetadataTextEdit = QTextEdit()
        additionalMetadataForm.addRow("Additional Metadata",
                                      self.additionalAdditionalMetadataTextEdit)
...
##############################################################################
    def loadMetadataFromXml(self):
        ...
        self.tree = tree
# left side
        self.setFormTextValueByQuery(self.titleLineEdit,
                                     "idinfo/citation/citeinfo/title")
        self.setFormTextValueByQuery(self.custodianLineEdit,
                                     "idinfo/citation/citeinfo/origin")
        "Esri/DataProperties/coordRef/geogcsn"

        self.setFormTextValueByQuery(self.startingDateLineEdit,
                                     "idinfo/timeperd/begdate")
        self.setFormTextValueByQuery(self.endingDateLineEdit,
                                     "idinfo/timeperd/enddate")
        self.setFormTextValueByQuery(self.abstractTextEdit,
                                     "idinfo/descript/abstract")
# contact pane
        self.setFormTextValueByQuery(self.contactOrganisationLineEdit,
                                     "metainfo/metc/cntinfo/cntorgp/cntorg")
        self.setFormTextValueByQuery(self.contactPositionLineEdit,
                                     "metainfo/metc/cntinfo/cntorgp/cntper")
        self.setFormTextValueByQuery(self.contactMailAddressLineEdit,
                                     "metainfo/metc/cntinfo/cntaddr/addrtype")
        self.setFormTextValueByQuery(self.contactSuburbLineEdit,
                                     "metainfo/metc/cntinfo/cntaddr/city")
        self.setFormTextValueByQuery(self.contactCountryStateLineEdit,
                                     "metainfo/metc/cntinfo/cntaddr/state")
        self.setFormTextValueByQuery(self.contactPostcodeLineEdit,
                                     "metainfo/metc/cntinfo/cntaddr/postal")
        self.setFormTextValueByQuery(self.contactTelephoneLineEdit,
                                     "metainfo/metc/cntinfo/cntvoice")
        self.setFormTextValueByQuery(self.contactFaxLineEdit,
                                     "distinfo/distrib/cntinfo/cntfax")
        self.setFormTextValueByQuery(self.contactEmailLineEdit,
                                     "distinfo/distrib/cntinfo/cntemail")

# access pane
        self.setFormTextValueByQuery(self.accessStoredDataFormatLineEdit,
                                     "idinfo/citation/citeinfo/geoform")
        self.setFormTextValueByQuery(self.accessAccessConstraintsTextEdit,
                                     "idinfo/accconst")

# quality pane
        self.setFormTextValueByQuery(self.qualityLineageTextEdit,
                                     "dataqual/lineage")
        self.setFormTextValueByQuery(self.qualityPositionalAccuracyTextEdit,
                                     "dataqual/posacc")
        self.setFormTextValueByQuery(self.qualityAttributeAccuracyTextEdit,
                                     "dataqual/attracc")
        self.setFormTextValueByQuery(self.qualityLogicalConsistancyTextEdit,
                                     "dataqual/logic")
        self.setFormTextValueByQuery(self.qualityCompletenessTextEdit,
                                     "dataqual/complete")

# status pane
        self.setFormTextValueByQuery(self.statusProgressTextEdit,
                                     "idinfo/status/progress")
        self.setFormTextValueByQuery(self.statusMaintenanceUpdateTextEdit,
                                     "idinfo/status/update")

# additional pane
        self.setFormTextValueByQuery(self.geographicExtentLeftLineEdit,
                                     "idinfo/spdom/lbounding/leftbc")
        self.setFormTextValueByQuery(self.geographicExtentRightLineEdit,
                                     "idinfo/spdom/lbounding/rightbc")
        self.setFormTextValueByQuery(self.geographicExtentTopLineEdit,
                                     "idinfo/spdom/lbounding/topbc")
        self.setFormTextValueByQuery(self.geographicExtentBottomLineEdit,
                                     "idinfo/spdom/lbounding/bottombc")
        self.setFormTextValueByQuery(self.additionalMetadataDateLineEdit,
                                     "metainfo/metd/date")
        self.setFormTextValueByQuery(self.additionalAdditionalMetadataTextEdit,
                                     "metainfo/addmeta")
...
##############################################################################
    def updateTreeFromForm(self):
        # left side
            self.setXmlValueFromFormText(self.titleLineEdit,
                                         "idinfo/citation/citeinfo/title")
            self.setXmlValueFromFormText(self.custodianLineEdit,
                                         "idinfo/citation/citeinfo/origin")
            self.setXmlValueFromFormText(self.startingDateLineEdit,
                                         "idinfo/timeperd/begdate")
            self.setXmlValueFromFormText(self.endingDateLineEdit,
                                         "idinfo/timeperd/enddate")
            self.setXmlValueFromFormText(self.abstractTextEdit,
                                         "idinfo/descript/abstract")
# contact pane
            self.setXmlValueFromFormText(self.contactOrganisationLineEdit,
                                         "metainfo/metc/cntinfo/cntorgp/cntorg")
            self.setXmlValueFromFormText(self.contactPositionLineEdit,
                                         "metainfo/metc/cntinfo/cntorgp/cntper")
            self.setXmlValueFromFormText(self.contactMailAddressLineEdit,
                                         "metainfo/metc/cntinfo/cntaddr/addrtype")
            self.setXmlValueFromFormText(self.contactSuburbLineEdit,
                                         "metainfo/metc/cntinfo/cntaddr/city")
            self.setXmlValueFromFormText(self.contactCountryStateLineEdit,
                                         "metainfo/metc/cntinfo/cntaddr/state")
            self.setXmlValueFromFormText(self.contactPostcodeLineEdit,
                                         "metainfo/metc/cntinfo/cntaddr/postal")
            self.setXmlValueFromFormText(self.contactTelephoneLineEdit,
                                         "metainfo/metc/cntinfo/cntvoice")
            self.setXmlValueFromFormText(self.contactFaxLineEdit,
                                         "distinfo/distrib/cntinfo/cntfax")
            self.setXmlValueFromFormText(self.contactEmailLineEdit,
                                         "distinfo/distrib/cntinfo/cntemail")

# access pane
            self.setXmlValueFromFormText(self.accessStoredDataFormatLineEdit,
                                         "idinfo/citation/citeinfo/geoform")
            self.setXmlValueFromFormText(self.accessAccessConstraintsTextEdit,
                                         "idinfo/accconst")

# quality pane
            self.setXmlValueFromFormText(self.qualityLineageTextEdit,
                                         "dataqual/lineage")
            self.setXmlValueFromFormText(self.qualityPositionalAccuracyTextEdit,
                                         "dataqual/posacc")
            self.setXmlValueFromFormText(self.qualityAttributeAccuracyTextEdit,
                                         "dataqual/attracc")
            self.setXmlValueFromFormText(self.qualityLogicalConsistancyTextEdit,
                                         "dataqual/logic")
            self.setXmlValueFromFormText(self.qualityCompletenessTextEdit,
                                         "dataqual/complete")

# status pane
            self.setXmlValueFromFormText(self.statusProgressTextEdit,
                                         "idinfo/status/progress")
            self.setXmlValueFromFormText(self.statusMaintenanceUpdateTextEdit,
                                         "idinfo/status/update")

# additional pane
            self.setXmlValueFromFormText(self.geographicExtentLeftLineEdit,
                                         "idinfo/spdom/lbounding/leftbc")
            self.setXmlValueFromFormText(self.geographicExtentLeftLineEdit,
                                         "idinfo/spdom/bounding/westbc")
            self.setXmlValueFromFormText(self.geographicExtentRightLineEdit,
                                         "idinfo/spdom/lbounding/rightbc")
            self.setXmlValueFromFormText(self.geographicExtentRightLineEdit,
                                         "idinfo/spdom/bounding/eastbc")
            self.setXmlValueFromFormText(self.geographicExtentTopLineEdit,
                                         "idinfo/spdom/lbounding/topbc")
            self.setXmlValueFromFormText(self.geographicExtentTopLineEdit,
                                         "idinfo/spdom/bounding/northbc")
            self.setXmlValueFromFormText(self.geographicExtentBottomLineEdit,
                                         "idinfo/spdom/lbounding/bottombc")
            self.setXmlValueFromFormText(self.geographicExtentBottomLineEdit,
                                         "idinfo/spdom/bounding/southbc")
            self.setXmlValueFromFormText(self.additionalMetadataDateLineEdit,
                                         "metainfo/metd/date")
            self.setXmlValueFromFormText(self.additionalAdditionalMetadataTextEdit,
                                         "metainfo/addmeta")
...
# CRS
            ...
            self.setXmlValueFromString(crsHelper.geogcsn,
                                       "Esri/DataProperties/coordRef/geogcsn")
            self.setXmlValueFromString(crsHelper.geogcsn,
                                       "spref/horizsys/cordsysn/geogcsn")
            self.setXmlValueFromString(crsHelper.projcsn,
                                       "Esri/DataProperties/coordRef/projcsn")
            self.setXmlValueFromString(crsHelper.projcsn,
                                       "spref/horizsys/cordsysn/projcsn")

            if crsHelper.projected:
                self.setXmlValueFromString("Geographic",
                                           "Esri/DataProperties/coordRef/type")
            else:
                self.setXmlValueFromString("Projected",
                                           "Esri/DataProperties/coordRef/type")
...
##############################################################################
    def setXmlValueFromFormText(self, formElement, query):
        if type(formElement) is QLineEdit:
            ...
            value = str(formElement.toPlainText())
        else:
...
##############################################################################
    ...
    def contactsPresetsComboBoxHandler(self, i):
        ...
        if key in self.contactsDictonary:
            ...
            confirm = Tools.showYesNoDialog("Do you want to load contact " +
                                            "details for " + contact.title +
                                            "?", "Overwrite Contacts")
            if confirm:
