    ...
    def zoomClickHandler(self):
        if self.localityRadioButton.isChecked():
            if self.localityLineEdit.text() == "":
                ...
                lat = self.model.itemFromIndex(self.localityTable.selectedIndexes()[2]).text()
                lon = self.model.itemFromIndex(self.localityTable.selectedIndexes()[3]).text()
            ...
            if self.latEastingLineEdit.text() == "" or self.lonNorthingLineEdit.text() == "":
                ...
                if self.crsComboBox.currentIndex() == 0:    # i.e. Lat/Long
                    lat = self.latEastingLineEdit.text()
                    lon = self.lonNorthingLineEdit.text()
                    # Check that input lat/long have been entered in correct format
                    if (not lat.replace("-", "").isnumeric()) or (not lon.replace("-", "").isnumeric()):    # isnumeric gives false for -ve numbers
                        QMessageBox.information(None, "Incorrect Format!", 
                                                "Lat/Long must be in decimal degrees - e.g. Lat = -32.123, Long = 115.789")
                        if not lat.replace("-", "").isnumeric():
                            self.latEastingLineEdit.setFocus()
                        elif not lon.replace("-", "").isnumeric():
                            self.lonNorthingLineEdit.setFocus()
                        return
                    if float(lat) > 90 or float(lat) < -90:
                        QMessageBox.information(None, "Invalid Latitude!", 
                                                "Latitude must be between -90 and 90")
                        self.latEastingLineEdit.setText("")
                        self.latEastingLineEdit.setFocus()
                        return
                    if float(lon) > 180 or float(lon) < -180:
                        QMessageBox.information(None, "Invalid Longitude!", 
                    ...
                    if float(lat) > 0:
                        reply = QMessageBox.question(self, 'Check latitude', 
                                                    'Latitudes for Western Australia are negative.  Do you want to use latitude -' + lat + " instead?", 
                                                    QMessageBox.Yes, QMessageBox.No, QMessageBox.Cancel)
                        if reply == QMessageBox.Yes:
                            self.latEastingLineEdit.setText("-" + lat)
                            lat = "-" + lat
                        elif reply == QMessageBox.Cancel:
                            return
                    if float(lon) < 0:
                        reply = QMessageBox.question(self, 'Check longitude', 
                                                    'Longitudes for Western Australia are positive.  Do you want to use longitude ' + lon[1:] + " instead?", 
                                                    QMessageBox.Yes, QMessageBox.No, QMessageBox.Cancel)
                        if reply == QMessageBox.Yes:
                            self.lonNorthingLineEdit.setText(lon[1:])
                            lon = lon[1:]
                        elif reply == QMessageBox.Cancel:
                            return                            
                    self.zoomToLatLon(lat, lon)
                elif self.crsComboBox.currentIndex() == 1:    # i.e. MGA Easting/Northing
                    east = self.latEastingLineEdit.text()
                    north = self.lonNorthingLineEdit.text()
                    # Check that input coords have been entered in correct format
                    if (not east.replace("-", "").isnumeric()) or (not north.replace("-", "").isnumeric()):    # isnumeric gives false for -ve numbers
                        QMessageBox.information(None, "Incorrect Format!", 
                                                "Easting/Northing must be numeric")
                        if not east.replace("-", "").isnumeric():
                            self.latEastingLineEdit.setFocus()
                        elif not north.replace("-", "").isnumeric():
                            self.lonNorthingLineEdit.setFocus()
                        return
                    if float(east) < 0 or float(east) > 500000:
                        reply = QMessageBox.question(self, 'Check easting', 
                                                    'Eastings are normally between 0 and 500,000.  Do you want to continue with easting ' + east + '?', 
                                                    QMessageBox.Yes, QMessageBox.No)
                        if reply == QMessageBox.No:
                            self.latEastingLineEdit.setFocus()
                            return
                    if float(north) < 6000000 or float(north) > 8600000:
                        reply = QMessageBox.question(self, 'Check northing', 
                                                    'Northings in WA are normally between 6,000,000 and 8,600,000.  Do you want to continue with northing ' + north + '?', 
                                                    QMessageBox.Yes, QMessageBox.No)
                        if reply == QMessageBox.No:
                            self.lonNorthingLineEdit.setFocus()
                            return                                
