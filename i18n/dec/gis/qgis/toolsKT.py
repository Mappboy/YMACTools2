...
class Tools():
    ...
    #projectRead = False    # Initially used in cddpTechnique decision but no longer required - delete if no problems arise by 28/2/15
...
###################################
    ...
    def findData(relativeLocation, zone=""):
        ...
        # pine platations imagery tile index has the datapath stored in the
...
#############################
    ...
    def updateCounter(cddpLayer):
        ...
        try:
            user = Tools.machineName
            #try:
            #    ip = str(socket.gethostbyname(socket.gethostname()))
            #except:
            #    ip = "unknown"
            date = time.strftime("%Y%m%d")
            string = cddpLayer + ", " + user + ", " + Tools.corporateDataDrive + ", " + date + ", QGIS, " + Tools.cddpTechnique + "\n"
    ...
    @staticmethod
    def updateWMSCounter(wmsLayer, wmsInCDDP):
        # get path to counter.txt
        folder = os.path.abspath(Tools.localLogFolder)
        ...
        try:
            user = Tools.machineName
            date = time.strftime("%Y%m%d")
            string = wmsLayer + ", " + user + ", " + Tools.corporateDataDrive + ", " + date + ", QGIS, " + Tools.cddpTechnique + wmsInCDDP + "\n"
            # open counter.txt in 'append' mode:
